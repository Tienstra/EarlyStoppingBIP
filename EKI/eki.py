...
class EKI:
    ...
    def _sample_prior(self):
        ...
        ensemble = np.random.normal(
            loc=0, scale=1, size=(self.dim_particles, self.num_particles)
        )
    ...
    def compute_kalman_gain(self):
        ...
        CM = np.cov(self.ensemble, self._push_forward())[
            self.dim_particles : (self.dim_particles + self.dim_observations),
            0 : self.dim_particles,
        ].T
        # emperical covariance of g_theta
        Sigma = np.cov(self._push_forward())
        # Kalman gain
        K = CM @ np.linalg.inv(
            (Sigma + (1 / self._get_dt()) * self.generate_noise_cov())
        )
...
if __name__ == "__main__":
    ...
    EKI = EKI(
        linear_model, y, dim_y, num_particles, prior_cov, init_mean, noise_level, time
    )
    print(EKI.ensemble)
